## 2.3.3（2026-09-12）— 面板可用性与启动性能修复（僵尸引擎 / 启动阻塞 / 首屏探活）

用户实测暴露三件事：①「A 服务配置 / O 邮箱注册台」打开空白或一直转圈；
②「项目启动打开很慢」；③界面首次状态刷新明显卡顿。根因与修复如下：

- **[P1] 面板空白/转圈 = 桥接指向「僵尸引擎」**：上一实例崩溃或被强杀后，
  其 A(:8000)/O(:8890) 进程可能仍占着端口却不再响应。`embedded_backends`
  旧逻辑「端口被占用即认为引擎已就绪并跳过启动」，于是主控制台桥接的
  `/aar-api`、`/oar-api` 全指向死引擎 → 一致返回 502 → 面板空白/转圈；
  反复双击会让问题反复出现。修复：端口占用时先做**健康探测** —— 健康则
  复用，无响应则按端口查 PID 杀掉后重启（新增 `_engine_health_ok` /
  `_pid_for_port` / `_kill_pid` / `_launch_or_reuse`；`netstat` 以 bytes
  读取，规避中文代码页的 `UnicodeDecodeError`）。
- **[P1] 面板在引擎冷启动期间不会自愈**：引擎首次解压需 1–3 分钟冷编译，
  期间面板请求失败后既不重试也不提示。新增前端自动重试：O 注册台
  `oarApiRetry`（40 次 × 5s）、A 服务配置 `aarApiRetry` /
  `aarSettingsLoadWithRetry`（30 次 × 4s）；引擎就绪后面板自动填充，
  不再永久转圈。
- **[P1] 启动慢 = 启动事件被同步探活阻塞**：`startup_aar_backend` 里的
  `await ensure_aar_running` 会同步探活等待（最坏约 20s），而 uvicorn 必须
  跑完所有 startup 事件才开始对外服务，于是这段等待被全额加到「双击 exe →
  窗口可用」上。修复：改为 fire-and-forget（`asyncio.create_task` + 异常
  保护 + 强引用 `_AAR_ENSURE_TASK`），uvicorn 立即对外服务；并在 shutdown
  事件里取消该后台任务，避免在途任务告警。实测引擎不可用/慢场景启动
  快约 4.2–4.7s。
- **[P2] 首屏状态刷新慢 = 串行探活叠加连接超时**：`/api/status` 会**串行**
  探测 BitBrowser 与 K12；而对「已关闭的 loopback 端口」建连要约 2s 才
  失败，两项叠加使单次请求要 3.6s。修复：探活超时 3s/1.5s 收到 0.8s、
  结果按短 TTL 缓存、两探活并行预热（`_probe_local_cached`）。实测首屏
  3.59s → 1.74s，后续每次约 0.03s。
- **[P2] `.env` 在冻结包里不生效**：冻结二进制不调用 `load_dotenv`，`.env`
  不会自动进 `os.environ`；默认 `PROXY_MODE=clash_auto` 会触发 6s Clash
  探测，超过桌面端 2s 健康检查超时，控制台因此打不开。修复：WebUI 导入期
  把 `.env` 的 `PROXY_MODE` 注入 `os.environ`（仅当进程未显式设置该变量）。
- **[发布工程] 发布闸门加固**：`tools/release/assert_release_artifact.py` 的
  卫生检查原先只拦「顶层 `.log`」，漏掉嵌套日志与 `.db` / `.db-wal` /
  `.db-shm` / `.sqlite` / `*.lock` / `runtime/state/**` 等运行期产物。本次打包
  实测确有 6 个运行期文件（A/O 账号库、SMS 池状态、锁）混入暂存包而闸门仍报
  75/75 PASS。现改为按「扩展名 + 路径」一票否决，并把判定抽成可测纯函数
  `runtime_artifact_hits()`；已核对官方 2.3.2 原始包上述计数恒为 0（不误伤
  `.env.example` 等随包模板），并补齐正/反向单测。

> 说明：以上修复位于 `webui/*`（面板后端与前端；便携包内以明文影子模块
> 生效）。`engine/` 下的 A/O 引擎为随包携带的独立子项目，不在本仓库内。

## 2.3.2（2026-09-11）— 全新解压包首次启动体验修复（冷启动看门狗误杀）

用户实测暴露：全新解压的 2.3.1 包首次双击，「A 服务配置 / O 邮箱注册台等
功能都不能用」。三因叠加，全部修复：

- **[P1] O 注册台被 30s 看门狗误杀**：官方 `embedded_backends._serve` 固定
  30s 看门狗，而全新解压包 `engine/oar` 无 `__pycache__`，OAR 重 import
  （outlook_api_reg 全家桶 + playwright/curl_cffi 冷编译 + 杀软逐文件扫描）
  实测超过 100s —— 看门狗超时判死且**永不重试**，O 注册台表现为「不可用」。
  影子修复版（`webui/embedded_backends.py` v4，wrapper_entry v4 纳入影子清单）：
  看门狗延长到 180s、每 15s 输出等待进度（提示冷编译需 1-3 分钟）。
- **[P1] 面板启动被串行拖住 30-60s**：官方 `try_start_embedded` 同步串行等
  A、O 各 30s，主面板跟着白屏，授权登录随之迟到约 1 分钟，期间所有功能请求
  被授权门禁 402 拦截 —— 用户视角即「全都坏了」。v4 改为 A/O **并行后台启动**、
  立即返回（面板秒开），后端就绪由 `status()` 懒重试兜底（冷却 20s，
  每引擎最多补 3 次，端口已被外部实例占用则视为就绪不再折腾）。
- **[P2] 全新包缺用户数据**：zip 不含 `_internal/.env`（用户 A 服务配置/凭证）。
  2.3.2 起 `build_local` 迁移清单即权威数据迁移路径；公开 zip 依旧不带
  用户数据（防泄漏），但发布说明明确「从旧目录迁移 5 项数据」。
- **[P2] 发布 zip 预编译 pyc**：打包时对 `engine/aar`、`engine/oar` 以内嵌
  同版本解释器预编译 `__pycache__` 并纳入 zip，全新解压首次启动不再冷编译
  （pyc 失配时 Python 自动回退现场编译，无副作用）。
- 防回归：`CONTENT_GUARDS` 锁定影子版 embedded_backends 关键符号；
  `test_wrapper_shadow_modules` 锁定影子清单顺序；新增 v4 行为测试
  （立即返回、并行回写、懒重试自愈、预算封顶）。
