...
async function pollStatus(){
  try{
    ...
    const online = [s.bitbrowser, networkOnline, s.chatgpt_plus].filter(Boolean).length;
...
async function showView(v){
  ...
  $('#view-dashboard').style.display = v==='dashboard' ? 'block' : 'none';
  ...
  $('#view-tasks').style.display = v==='tasks' ? 'block' : 'none';
  $('#view-accounts').style.display = v==='accounts' ? 'block' : 'none';
  $('#view-settings').style.display = v==='settings' ? 'block' : 'none';
  $('#view-scripts').style.display = v==='scripts' ? 'block' : 'none';
  $('#view-oar').style.display = v==='oar' ? 'block' : 'none';
  if(v==='oar' && typeof initOarView==='function') initOarView();
  ['aarregister','aarproxies','aarsettings'].forEach(name=>{
    const el = $('#view-'+name); if(el) el.style.display = v===name ? 'block' : 'none';
  });
  $('#view-embed').style.display = v==='embed' ? 'block' : 'none';
  $$('.navbtn').forEach(b=>b.classList.toggle('active', b.dataset.view===v));
  if(!['run','embed','tasks','accounts','settings','scripts','aarregister','aarproxies','aarsettings'].includes(v)) $$('.scriptbtn').forEach(b=>b.classList.remove('active'));
  ...
  const loaders = {
    ...
    scripts:()=>{ renderScriptsTabs(); return Promise.resolve(); },
    aarregister:initAarRegister,
    aarproxies:loadAarProxies,
    aarsettings:initAarSettings,
  };
...
async function loadAssetPanel(){
  ...
  initMailboxInventory();
}

/* ===== 邮箱资产清单：直接列出本机全部四段凭据（含密码），无视领取状态 ===== */
let mailboxItems = [];
function esc(s){ return String(s ?? "").replace(/[&<>"]/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); }
function initMailboxInventory(){
  $('#btn-refresh-mailbox').onclick = loadMailboxInventory;
  $('#mailbox-search').oninput = renderMailbox;
  $('#mailbox-provider').onchange = renderMailbox;
  $('#mailbox-only-claimed').onchange = renderMailbox;
  $('#mailbox-hide-pw').onchange = renderMailbox;
  $('#mailbox-selall').onchange = (e)=>{ document.querySelectorAll('#mailbox-rows .mb-chk').forEach(b=>b.checked=e.target.checked); };
  $('#btn-mailbox-export-sel').onclick = ()=>exportMailbox(selectedMailbox());
  $('#btn-mailbox-export-all').onclick = ()=>exportMailbox(filteredMailbox());
  loadMailboxInventory();
}
async function loadMailboxInventory(){
  setAssetMessage('#mailbox-msg', '正在读取本机邮箱…');
  try{
    const r = await fetch('/api/assets/mailbox-inventory', {headers:assetHeaders()});
    const d = await readAssetResponse(r);
    mailboxItems = d.items || [];
    $('#mailbox-msg').textContent = `共 ${mailboxItems.length} 条`;
    renderMailbox();
  }catch(e){ setAssetMessage('#mailbox-msg', e.message||String(e), false); }
}
function filteredMailbox(){
  const q = $('#mailbox-search').value.trim().toLowerCase();
  const pv = $('#mailbox-provider').value;
  const onlyClaimed = $('#mailbox-only-claimed').checked;
  return mailboxItems.filter(it=>
    (!q || it.email.toLowerCase().includes(q)) &&
    (!pv || it.email_provider===pv) &&
    (!onlyClaimed || it.claimed)
  );
}
function renderMailbox(){
  const hide = $('#mailbox-hide-pw').checked;
  const rows = filteredMailbox();
  const tb = $('#mailbox-rows');
  tb.innerHTML = rows.map((it)=>{
    const pw = hide ? (it.password ? '•'.repeat(Math.min(16, it.password.length)) : '') : (it.password||'');
    const rt = hide ? (it.refresh_token ? '•••••••(已隐藏)' : '') : (it.refresh_token||'');
    return `<tr>
      <td><input type="checkbox" class="mb-chk"></td>
      <td class="mono">${esc(it.email)}</td>
      <td class="mono mailbox-password-cell"><span>${esc(pw)}</span><button type="button" class="password-toggle row-password-toggle" title="显示密码" aria-label="显示密码">👁</button></td>
      <td class="mono">${esc(rt)}</td>
      <td class="mono">${esc(it.client_id||'')}</td>
      <td><span class="tag">${esc(it.email_provider)}</span></td>
      <td>${it.claimed ? '<span class="tag" style="background:#3a2a13;color:#ffb454">已领取</span>' : '<span class="tag" style="background:#13322a;color:#5fd39a">在池</span>'}</td>
      <td class="src">${esc(it.source)}</td>
      <td class="actions">
        <button class="ghost" onclick="copyMailbox('${esc(it.email)}')">复制四段</button>
        <button class="ghost" onclick="downloadMailbox('${esc(it.email)}')">下载</button>
      </td>
    </tr>`;
  }).join("");
  $('#mailbox-empty').style.display = rows.length ? 'none' : 'block';
}
$('#mailbox-rows').addEventListener('click', e=>{
  const button=e.target.closest('.row-password-toggle');
  if(!button) return;
  const rowIndex=button.closest('tr').rowIndex-1;
  const row=filteredMailbox()[rowIndex];
  const text=button.parentElement.querySelector('span');
  if(!row || !text) return;
  const shown=button.dataset.shown==='1';
  text.textContent=shown?(row.password?'•'.repeat(Math.min(16,row.password.length)):''):(row.password||'');
  button.dataset.shown=shown?'0':'1';
  button.textContent=shown?'👁':'🙈';
  button.title=shown?'显示密码':'隐藏密码';
});
function mailboxLine(it){ return [it.email, it.password, it.refresh_token, it.client_id].join('----'); }
function selectedMailbox(){
  const boxes = document.querySelectorAll('#mailbox-rows .mb-chk');
  const rows = filteredMailbox();
  const sel = [];
  boxes.forEach((b,idx)=>{ if(b.checked) sel.push(rows[idx]); });
  return sel;
}
...
function exportMailbox(list){
  if(!list.length){ setAssetMessage('#mailbox-msg', '没有选中任何邮箱', false); return; }
  const fmt = $('#mailbox-fmt').value;
  if(fmt==='json'){
    downloadBlob(`rf-mailbox-${Date.now()}.json`, JSON.stringify(list.map(it=>({email:it.email,password:it.password,refresh_token:it.refresh_token,client_id:it.client_id,email_provider:it.email_provider,source:it.source})), null, 2));
  }else{
    downloadBlob(`rf-mailbox-${Date.now()}.txt`, list.map(mailboxLine).join('\n')+'\n');
  }
  setAssetMessage('#mailbox-msg', `已导出 ${list.length} 条`);
}
window.copyMailbox = (email)=>{ const it = mailboxItems.find(x=>x.email===email); if(it) navigator.clipboard.writeText(mailboxLine(it)); };
window.downloadMailbox = (email)=>{ const it = mailboxItems.find(x=>x.email===email); if(!it) return; const fmt=$('#mailbox-fmt').value; if(fmt==='json') downloadBlob(`${email}.json`, JSON.stringify({email:it.email,password:it.password,refresh_token:it.refresh_token,client_id:it.client_id},null,2)); else downloadBlob(`${email}.txt`, mailboxLine(it)+'\n'); };
...
bindPasswordToggle(document.querySelector('[data-password-target="asset-api-key"]'), $('#asset-api-key'));
const mailboxToggle=$('#btn-mailbox-toggle-pw');
if(mailboxToggle) mailboxToggle.onclick=()=>{
  const hide=$('#mailbox-hide-pw');
  hide.checked=!hide.checked;
  mailboxToggle.textContent=hide.checked?'👁 显示全部':'🙈 隐藏全部';
  renderMailbox();
};
...
async function loadScripts(){
  ...
  try{ EMBEDS = (await (await fetch('/api/embeds')).json()).embeds || []; }catch(err){ EMBEDS = []; }
  $('#task-count').textContent = SCRIPTS.length;
  renderScriptsTabs();
}

function renderScriptsTabs(){
  const cats = ['全部', ...new Set(SCRIPTS.map(s=>s.category))];
  if(EMBEDS.length) cats.push('辅助功能');
  const tabs = $('#scripts-tabs'); if(!tabs) return;
  tabs.innerHTML = cats.map(c=>`<button class="${c===curScriptCat?'active':''}" data-cat="${esc(c)}">${esc(c)}</button>`).join('');
  $$('#scripts-tabs button').forEach(b=>b.onclick=()=>{ curScriptCat=b.dataset.cat; renderScriptsTabs(); });
  renderScriptsGrid();
}

function renderScriptsGrid(){
  const grid = $('#scripts-grid'); if(!grid) return;
  const q = ($('#scripts-search')?.value||'').trim().toLowerCase();
  let items = [];
  if(curScriptCat==='全部' || curScriptCat==='辅助功能'){
    // 全部：脚本 + 辅助 + 外部链接混排；辅助功能：只显示嵌入工具
    if(curScriptCat==='辅助功能'){
      items = EMBEDS.map(e=>({type:'embed', id:e.id, title:e.title, desc:e.url}));
    }else{
      items = SCRIPTS.map(s=>({type:'script', id:s.id, title:s.title, desc:s.desc, cat:s.category, warning:s.warning}));
    }
  }else{
    items = SCRIPTS.filter(s=>s.category===curScriptCat).map(s=>({type:'script', id:s.id, title:s.title, desc:s.desc, cat:s.category, warning:s.warning}));
  }
  if(q) items = items.filter(i=>(i.title+' '+(i.desc||'')).toLowerCase().includes(q));
  $('#scripts-empty').style.display = items.length ? 'none' : 'block';
  grid.innerHTML = items.map(it=>`<button class="script-card ${it.type==='embed'?'card-embed':''}" data-type="${it.type}" data-id="${esc(it.id)}">
    <span class="script-card-title">${esc(it.title)}${it.type==='embed'?' <em>↗</em>':''}</span>
    ${it.desc?`<span class="script-card-desc">${esc(it.desc.length>60?it.desc.slice(0,60)+'…':it.desc)}</span>`:''}
  </button>`).join('');
  $$('#scripts-grid .script-card').forEach(card=>{
    card.onclick = ()=>{
      if(card.dataset.type==='embed'){ openEmbed(card.dataset.id); return; }
      showView('run'); selectScript(card.dataset.id);
    };
  });
}
$('#scripts-search') && ($('#scripts-search').oninput = ()=>renderScriptsGrid());

async function loadScriptsLegacySidebar(){/* 已改为任务库页渲染 */}
...
async function loadEnv(){
  ...
  data.groups.forEach(g=>{
    if(/k12/i.test(g.group)) return;  // 已下线 Codex K12 控制台分组
...
/* ===== 顶部全局搜索：过滤任务库页 + 侧边栏工具按钮 ===== */
$('#global-search').addEventListener('input', (e)=>{
  const q = e.target.value.trim().toLowerCase();
  // 任务库页有内容时同步它的搜索框并重渲染
  const sInput = $('#scripts-search');
  if(sInput){ sInput.value = e.target.value; renderScriptsGrid(); }
  // 同时过滤侧边栏工具按钮
  document.querySelectorAll('.sidebar-tools .navbtn').forEach(b=>{
...
async function loadDashboard(){
  // /api/status 里探测浏览器/Clash 要好几秒，别让它拖住整个首屏：
  // 快接口（邮箱资产）先渲染数字，状态徽章异步补。
  const statusPromise = fetch('/api/status').then(r=>r.json()).catch(()=>null);
  try{
    const [sum, mb] = await Promise.all([
      fetch('/api/assets/summary').then(r=>r.json()),
      fetch('/api/assets/mailbox-inventory', {headers:assetHeaders()}).then(r=>readAssetResponse(r)).then(d=>d.items||[]).catch(()=>[]),
    ]);
    var status = null;
    const plats = sum.platforms || {};
    const cookieTotal = Object.values(plats).reduce((a,p)=>a + (p.cookies||0) + (p.sessions||0), 0);
    const exported = (sum.lifecycle && sum.lifecycle.exported) ? sum.lifecycle.exported.accounts : 0;
    const claimed = (sum.claims && sum.claims.outlook) || 0;
    $('#stat-emails').textContent = sum.emails ?? '--';
    $('#stat-emails-foot').textContent = `已领取 ${claimed} · 在池 ${Math.max((sum.emails||0)-claimed,0)}`;
    $('#stat-cookies').textContent = cookieTotal;
    $('#stat-exported').textContent = exported;
    dashMailbox = mb;
    $('#dash-mailbox-count').textContent = `${mb.length} 个`;
    renderDashMailbox();
    loadAarStats();
  }catch(e){
  ...
  try{
    ...
    if(status) renderDashStatus(status);
  }catch(_){ }
}
function renderDashStatus(status){
  if($('#h-browser')) $('#h-browser').outerHTML = badge(status.bitbrowser ? '在线' : '离线', !!status.bitbrowser).replace('class="badge','id="h-browser" class="badge');
  if($('#h-network')) $('#h-network').outerHTML = badge(status.network ? '已连接' : '未连接', !!status.network).replace('class="badge','id="h-network" class="badge');
  if($('#h-plus')) $('#h-plus').outerHTML = badge(status.chatgpt_plus ? '可用' : '未检测', !!status.chatgpt_plus).replace('class="badge','id="h-plus" class="badge');
  if($('#h-version')) $('#h-version').outerHTML = badge(status.version || '--', true).replace('class="badge','id="h-version" class="badge');
  if($('#h-node')) $('#h-node').outerHTML = badge(status.node || '直连', true).replace('class="badge','id="h-node" class="badge');
  const healthDot = $('#health-dot'), healthLabel = $('#health-label');
  if(healthDot && healthLabel){
    healthDot.className = 'dot ' + (status.bitbrowser ? 'ok' : 'bad');
    healthLabel.textContent = status.bitbrowser ? '服务在线' : '浏览器离线';
  }
}
function renderDashMailbox(){
  const q = ($('#dash-mailbox-search').value||'').trim().toLowerCase();
  const rows = dashMailbox.filter(it=>!q || it.email.toLowerCase().includes(q));
  const tb = $('#dash-mailbox-rows');
  if(!tb) return;
  tb.innerHTML = rows.map(it=>{
    const pw = it.password ? '•'.repeat(Math.min(12, it.password.length)) : '';
    const st = it.claimed ? '<span class="tag warn">已领取</span>' : '<span class="tag ok">在池</span>';
    return `<tr><td class="mono">${esc(it.email)}</td><td class="mono">${esc(pw)}</td><td><span class="tag">${esc(it.email_provider)}</span></td><td>${st}</td></tr>`;
  }).join('') || '<tr><td colspan="4" class="dash-empty">暂无邮箱资产</td></tr>';
}
$('#btn-dash-refresh').onclick = loadDashboard;
$('#dash-mailbox-search').oninput = renderDashMailbox;
$('#btn-dash-mailbox-export').onclick = function(){
  if(!dashMailbox.length){ alert('没有邮箱可导出'); return; }
  const content = dashMailbox.map(it=>[it.email,it.password,it.refresh_token,it.client_id].join('----')).join('\n')+'\n';
  const blob = new Blob([content], {type:'text/plain;charset=utf-8'});
  const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`rf-mailbox-${Date.now()}.txt`; a.click();
};
// 默认首次加载总览
loadDashboard();

/* ===== 任务页（仿 AAR /history：用现有运行态接真数据） ===== */
async function loadTasks(){
  try{
    const status = await fetch('/api/status').then(r=>r.json());
    $('#task-m-running').textContent = status.running ?? 0;
    $('#task-m-pid').textContent = status.pid ?? '--';
    $('#task-m-script').textContent = status.running ? '注册中' : '空闲';
    $('#task-m-last').textContent = status.running ? '运行中' : '无';
    const rows = [];
    if(status.running){
      rows.push({time:'—', script:status.running_script||'当前注册任务', status:'运行中', pid:status.pid||'—'});
    }
    const q = ($('#tasks-search').value||'').trim().toLowerCase();
    const tb = $('#tasks-rows');
    tb.innerHTML = (rows.filter(r=>!q || r.script.toLowerCase().includes(q))).map(r=>
      `<tr><td class="mono">${esc(r.time)}</td><td>${esc(r.script)}</td><td><span class="tag ${r.status==='运行中'?'warn':'ok'}">${esc(r.status)}</span></td><td class="mono">${esc(r.pid)}</td><td><button class="btn-inline" onclick="showView('run')">查看日志</button></td></tr>`
    ).join('') || '';
    $('#tasks-empty').style.display = rows.length ? 'none' : 'block';
    loadAarTasks();
  }catch(e){ console.error('loadTasks failed', e); }
}
$('#btn-tasks-refresh').onclick = loadTasks;
$('#tasks-search').oninput = loadTasks;

/* ===== 账号资产页（仿 AAR /accounts：查邮箱资产真数据） ===== */
let accList = [];
async function loadAccounts(){
  try{
    const d = await fetch('/api/assets/mailbox-inventory', {headers:assetHeaders()}).then(r=>readAssetResponse(r)).then(d=>d.items||[]);
    accList = d;
    const cnt = {outlook:0, icloud:0, other:0};
    accList.forEach(it=>{ const p=(it.email_provider||'').toLowerCase(); if(p==='outlook')cnt.outlook++; else if(p==='icloud')cnt.icloud++; else cnt.other++; });
    $('#acc-total').textContent = accList.length;
    $('#acc-outlook').textContent = cnt.outlook;
    $('#acc-icloud').textContent = cnt.icloud;
    $('#acc-other').textContent = cnt.other;
    renderAccounts();
    loadAarPlatforms();
  }catch(e){ console.error('loadAccounts failed', e); $('#accounts-empty').style.display='block'; }
}
function renderAccounts(){
  const q = ($('#accounts-search').value||'').trim().toLowerCase();
  const rows = accList.filter(it=>!q || it.email.toLowerCase().includes(q));
  const tb = $('#accounts-rows');
  if(!tb) return;
  tb.innerHTML = rows.map((it,i)=>{
    const st = it.claimed ? '<span class="tag warn">已领取</span>' : '<span class="tag ok">在池</span>';
    const pw = it.password||'';
    return `<tr>
      <td><input type="checkbox" class="acc-chk" data-i="${i}"></td>
      <td class="mono">${esc(it.email)}</td>
      <td class="mono acc-pw" data-pw="${esc(pw)}">${pw?'••••••••':''}</td>
      <td><button class="btn-inline pw-eye" title="显示/隐藏密码">👁</button></td>
      <td class="mono small">${esc(it.refresh_token||'')}</td>
      <td class="mono small">${esc(it.client_id||'')}</td>
      <td><span class="tag">${esc(it.email_provider)}</span></td>
      <td>${st}</td>
      <td><button class="btn-inline" onclick="copyText('${esc(it.email)}')">复制邮箱</button></td>
    </tr>`;
  }).join('') || '';
  $('#accounts-empty').style.display = rows.length ? 'none' : 'block';
}
$('#btn-accounts-refresh').onclick = loadAccounts;
$('#accounts-search').oninput = renderAccounts;
$('#acc-selall').onchange = e=>{ $$('.acc-chk').forEach(c=>c.checked=e.target.checked); };
// 密码小眼睛：点击切显示/隐藏（事件委托，渲染后也生效）
$('#accounts-rows').addEventListener('click', e=>{
  const eye = e.target.closest('.pw-eye');
  if(!eye) return;
  const cell = eye.closest('tr')?.querySelector('.acc-pw');
  if(!cell) return;
...
$('#btn-accounts-export').onclick = ()=>{
  const picked = $$('.acc-chk').filter(c=>c.checked).map(c=>accList[+c.dataset.i]);
  const data = picked.length ? picked : accList;
  if(!data.length){ alert('没有账号可导出'); return; }
  const content = data.map(it=>[it.email,it.password,it.refresh_token,it.client_id].join('----')).join('\n')+'\n';
  const blob = new Blob([content], {type:'text/plain;charset=utf-8'});
  const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`rf-accounts-${Date.now()}.txt`; a.click();
};

/* ===== 设置中心（仿 AAR /settings 九宫格：跳转现有设置页） ===== */
function initSettings(){
  $$('#view-settings .settings-tile').forEach(t=>{
    t.onclick = ()=> showView(t.dataset.goto);
  });
}

/* ===== A · 全功能注册台（原生重写，调 /aar-api 真接口） ===== */
async function aarApi(path, opts={}){
  const r = await fetch('/aar-api'+path, {
...
const STATUS_LABELS = {
  ...
  running:'运行中', success:'成功', failed:'失败', cancelled:'已取消', interrupted:'已中断',
};

/* ---- A 总览（并入总览页：注册台账号卡 + 平台分布 + 状态分布） ---- */
async function loadAarStats(){
  try{
    const s = await aarApi('/accounts/stats');
    const el = $('#stat-aar-total');
    if(!el) return;
    el.textContent = s.total ?? 0;
    const plan = s.by_plan_state||{};
    $('#stat-aar-foot').textContent = `试用 ${plan.trial??0} · 订阅 ${plan.subscribed??0}`;
    const byPlat = s.by_platform||{};
    const entries = Object.entries(byPlat).sort((a,b)=>b[1]-a[1]);
    const total = entries.reduce((a,[,n])=>a+n,0) || 1;
    const platEl = $('#ah-platforms');
    if(platEl) platEl.innerHTML = entries.map(([p,n])=>{
      const pct = Math.round(n/total*100);
      return `<div class="health-row"><span class="h-label">${esc(p)}</span><span class="badge">${n} · ${pct}%</span></div>`;
    }).join('') || '<div class="dash-empty">暂无账号</div>';
    const group = (title, obj)=>`<div style="margin-bottom:10px"><div class="h-label" style="margin-bottom:6px">${esc(title)}</div>${
      Object.entries(obj||{}).map(([k,n])=>`<span class="badge" style="margin:2px 4px 2px 0">${STATUS_LABELS[k]||k}: ${n}</span>`).join('') || '<span class="dash-empty">无</span>'}</div>`;
    const sgEl = $('#ah-status-groups');
    if(sgEl) sgEl.innerHTML = group('套餐', s.by_plan_state) + group('生命周期', s.by_lifecycle_status) + group('有效性', s.by_validity_status);
  }catch(e){ console.error('loadAarStats', e); }
}

/* ---- A 账号管理 ---- */
let aaPlatform='', aaPage=1, aaTotalPages=1, aaSelected=new Set(), aaItems=[];
async function loadAarPlatforms(){
  try{
    const list = await aarApi('/platforms');
    const opts = list.map(p=>`<option value="${esc(p.name)}">${esc(p.display_name)}</option>`).join('');
    $('#aa-platform').innerHTML = opts || '<option value="">无平台</option>';
    $('#ar-platform').innerHTML = opts;
    $('#at-platform').innerHTML = '<option value="">全部平台</option>'+opts;
    if(list.length){ aaPlatform = list[0].name; loadAarAccounts(); }
  }catch(e){ console.error('loadAarPlatforms', e); }
}
async function loadAarAccounts(){
  try{
    const plat = $('#aa-platform').value || aaPlatform;
    aaPlatform = plat;
    if(!plat) return;
    const q = new URLSearchParams({platform:plat, page:aaPage, page_size:20});
    const st = $('#aa-status').value; if(st) q.set('status', st);
    const kw = ($('#aa-search').value||'').trim(); if(kw) q.set('email', kw);
    const d = await aarApi('/accounts?'+q.toString());
    aaItems = d.items||[];
    aaTotalPages = Math.max(1, Math.ceil((d.total||0)/20));
    $('#aa-count').textContent = `${d.total||0} 个`;
    $('#aa-page').textContent = `${aaPage} / ${aaTotalPages}`;
    $('#aa-sub').textContent = plat ? `平台：${plat}` : '全部平台';
    renderAaRows();
  }catch(e){ console.error('loadAarAccounts', e); }
}
function renderAaRows(){
  const tb = $('#aa-rows');
  tb.innerHTML = aaItems.map(it=>{
    const stCls = {trial:'warn', subscribed:'ok', invalid:'bad', expired:'bad'}[it.lifecycle_status]||'';
    const st = `<span class="tag ${stCls}">${STATUS_LABELS[it.lifecycle_status]||it.lifecycle_status||'--'}</span>`;
    return `<tr>
      <td><input type="checkbox" class="aa-chk" data-id="${it.id}"></td>
      <td class="mono">${esc(it.email||'')}</td>
      <td class="mono">${esc(it.password?'•'.repeat(Math.min(10,it.password.length)):'')}</td>
      <td>${st}</td>
      <td class="mono small">${esc((it.created_at||'--').slice(0,19))}</td>
      <td><button class="btn-inline" onclick="aaDel(${it.id})">删除</button></td>
    </tr>`;
  }).join('');
  $('#aa-empty').style.display = aaItems.length ? 'none' : 'block';
  $('#btn-aa-delete').style.display = aaSelected.size ? 'inline-block' : 'none';
}
window.aaDel = async id=>{
  if(!confirm('确认删除该账号？')) return;
  await aarApi(`/accounts/${id}`, {method:'DELETE'});
  loadAarAccounts();
};
$('#aa-platform').onchange = ()=>{ aaPage=1; aaSelected.clear(); loadAarAccounts(); };
$('#aa-status').onchange = ()=>{ aaPage=1; loadAarAccounts(); };
let aaSearchTimer=null;
$('#aa-search').oninput = ()=>{ clearTimeout(aaSearchTimer); aaSearchTimer=setTimeout(()=>{ aaPage=1; loadAarAccounts(); },400); };
$('#btn-aa-refresh').onclick = loadAarAccounts;
$('#btn-aa-prev').onclick = ()=>{ if(aaPage>1){ aaPage--; loadAarAccounts(); } };
$('#btn-aa-next').onclick = ()=>{ if(aaPage<aaTotalPages){ aaPage++; loadAarAccounts(); } };
$('#aa-selall').onchange = e=>{
  $$('.aa-chk').forEach(c=>{ c.checked=e.target.checked; if(c.checked) aaSelected.add(+c.dataset.id); else aaSelected.delete(+c.dataset.id); });
  $('#btn-aa-delete').style.display = aaSelected.size ? 'inline-block' : 'none';
};
document.addEventListener('change', e=>{
  if(e.target.classList && e.target.classList.contains('aa-chk')){
    if(e.target.checked) aaSelected.add(+e.target.dataset.id); else aaSelected.delete(+e.target.dataset.id);
    $('#btn-aa-delete').style.display = aaSelected.size ? 'inline-block' : 'none';
  }
});
$('#btn-aa-delete').onclick = async ()=>{
  if(!aaSelected.size) return;
  if(!confirm(`确认删除选中的 ${aaSelected.size} 个账号？`)) return;
  for(const id of aaSelected){ await aarApi(`/accounts/${id}`, {method:'DELETE'}).catch(()=>{}); }
  aaSelected.clear(); loadAarAccounts();
};
$('#btn-aa-add').onclick = ()=>$('#aa-add-dialog').showModal();
$('#btn-ad-save').onclick = async ()=>{
  try{
    await aarApi('/accounts', {method:'POST', body:JSON.stringify({
      platform:$('#aa-platform').value||aaPlatform,
      email:$('#ad-email').value.trim(),
      password:$('#ad-password').value,
      primary_token:$('#ad-token').value,
      cashier_url:$('#ad-cashier').value.trim(),
      lifecycle_status:$('#ad-status').value,
    })});
    $('#aa-add-dialog').close(); loadAarAccounts();
  }catch(e){ alert('新增失败：'+e.message); }
};
$('#btn-aa-import').onclick = ()=>$('#aa-import-dialog').showModal();
$('#btn-ai-save').onclick = async ()=>{
  try{
    await aarApi('/accounts/import', {method:'POST', body:JSON.stringify({
      platform:$('#aa-platform').value||aaPlatform,
      lines:($('#ai-lines').value||'').split('\n').map(s=>s.trim()).filter(Boolean),
    })});
    $('#aa-import-dialog').close(); loadAarAccounts();
  }catch(e){ alert('导入失败：'+e.message); }
};
$('#btn-aa-export').onclick = ()=>{
  const plat = $('#aa-platform').value||aaPlatform;
  if(!plat) return;
  window.open(`/aar-api/accounts/export?platform=${encodeURIComponent(plat)}`, '_blank');
};

/* ---- A 自动注册 ---- */
let arIdentity='', arExecutor='';
async function initAarRegister(){
  try{
    if(!$('#ar-platform option')) await loadAarPlatforms();
...
$('#ar-platform').onchange = ()=>{ arIdentity=''; arExecutor=''; initAarRegister(); };
$('#btn-ar-start').onclick = async ()=>{
  try{
    const d = await aarApi('/tasks/register', {method:'POST', body:JSON.stringify({
      platform:$('#ar-platform').value,
      count:+$('#ar-count').value||1,
      executor_type:arExecutor||undefined,
      proxy:$('#ar-proxy').value.trim()||undefined,
      extra:{identity_mode:arIdentity||undefined},
    })});
    $('#ar-task-badge').textContent = '运行中'; $('#ar-task-badge').className='badge ok';
    $('#ar-task-id').textContent = d.task_id||'--';
    pollArTask(d.task_id);
  }catch(e){ alert('启动失败：'+e.message); }
};
function pollArTask(id){
  clearInterval(window._arTimer);
  window._arTimer = setInterval(async ()=>{
    try{
      const t = await aarApi(`/tasks/${id}`);
      $('#ar-progress').textContent = `${t.progress_detail?.current??0}/${t.progress_detail?.total??t.progress??0}`;
      $('#ar-success-fail').textContent = `${t.success??0} / ${t.error_count??0}`;
      const ev = await aarApi(`/tasks/${id}/events?limit=30`);
      const lines = (ev.items||[]).map(e=>`[${e.created_at||''}] ${e.message||''}`).join('\n');
      const pre = $('#ar-log'); pre.textContent = lines || '暂无日志'; pre.scrollTop = pre.scrollHeight;
      if(['success','failed','cancelled','interrupted'].includes(t.status)){
        clearInterval(window._arTimer);
        $('#ar-task-badge').textContent = STATUS_LABELS[t.status]||t.status;
        $('#ar-task-badge').className = 'badge '+(t.status==='success'?'ok':'bad');
      }
    }catch(e){ console.error('pollArTask', e); }
  }, 3000);
}

/* ---- A 任务记录 ---- */
async function loadAarTasks(){
  try{
    const q = new URLSearchParams({page:1, page_size:50});
    if($('#at-platform').value) q.set('platform', $('#at-platform').value);
    if($('#at-status').value) q.set('status', $('#at-status').value);
    const d = await aarApi('/tasks?'+q.toString());
    const items = d.items||[];
    $('#at-total').textContent = items.length;
    $('#at-ok').textContent = items.filter(t=>t.status==='success').length;
    $('#at-fail').textContent = items.filter(t=>['failed','interrupted'].includes(t.status)).length;
    $('#at-running').textContent = items.filter(t=>['running','pending'].includes(t.status)).length;
    $('#at-rows').innerHTML = items.map(t=>`<tr>
      <td class="mono small">${esc((t.created_at||'--').slice(5,19))}</td>
      <td class="mono small">${esc(String(t.id).slice(0,8))}</td>
      <td><span class="tag">${esc(t.platform||'--')}</span></td>
      <td><span class="tag ${t.status==='success'?'ok':(t.status==='failed'||t.status==='interrupted'?'bad':'warn')}">${STATUS_LABELS[t.status]||t.status||'--'}</span></td>
      <td class="mono">${t.progress??0}%</td>
      <td class="mono">${t.success??0}/${t.error_count??0}</td>
      <td class="small" style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc((t.errors||[])[0]||'')}</td>
    </tr>`).join('');
    $('#at-empty').style.display = items.length ? 'none' : 'block';
  }catch(e){ console.error('loadAarTasks', e); }
}
$('#btn-at-refresh').onclick = loadAarTasks;
$('#at-platform').onchange = loadAarTasks;
$('#at-status').onchange = loadAarTasks;
$('#btn-at-clear').onclick = ()=>{ $('#at-platform').value=''; $('#at-status').value=''; loadAarTasks(); };

/* ---- A 代理池 ---- */
async function loadAarProxies(){
  try{
    const list = await aarApi('/proxies');
    $('#ap-total').textContent = list.length;
    $('#ap-active').textContent = list.filter(p=>p.is_active).length;
    $('#ap-ok').textContent = list.reduce((a,p)=>a+(p.success_count||0),0);
    $('#ap-fail').textContent = list.reduce((a,p)=>a+(p.fail_count||0),0);
    $('#ap-count').textContent = `${list.length} 条`;
    $('#ap-rows').innerHTML = list.map(p=>`<tr>
      <td class="mono small">${esc(p.url)}</td>
      <td><span class="tag">${esc(p.region||'--')}</span></td>
      <td class="mono">${p.success_count||0} / ${p.fail_count||0}</td>
      <td><span class="tag ${p.is_active?'ok':'bad'}">${p.is_active?'活跃':'停用'}</span></td>
      <td>
        <button class="btn-inline" onclick="apToggle('${p.id}')">${p.is_active?'停用':'启用'}</button>
        <button class="btn-inline" style="color:#f87171" onclick="apDel('${p.id}')">删除</button>
      </td>
    </tr>`).join('');
    $('#ap-empty').style.display = list.length ? 'none' : 'block';
  }catch(e){ console.error('loadAarProxies', e); }
}
window.apToggle = async id=>{ await aarApi(`/proxies/${id}/toggle`, {method:'PATCH'}); loadAarProxies(); };
window.apDel = async id=>{ if(confirm('删除该代理？')){ await aarApi(`/proxies/${id}`, {method:'DELETE'}); loadAarProxies(); } };
$('#btn-ap-add').onclick = async ()=>{
  const lines = ($('#ap-input').value||'').split('\n').map(s=>s.trim()).filter(Boolean);
  const region = $('#ap-region').value.trim()||null;
  try{
    if(lines.length===1){ await aarApi('/proxies', {method:'POST', body:JSON.stringify({url:lines[0], region})}); }
    else if(lines.length>1){ await aarApi('/proxies/bulk', {method:'POST', body:JSON.stringify({proxies:lines, region})}); }
    $('#ap-input').value=''; loadAarProxies();
  }catch(e){ alert('添加失败：'+e.message); }
};
$('#btn-ap-check').onclick = async ()=>{ await aarApi('/proxies/check', {method:'POST'}).catch(()=>{}); setTimeout(loadAarProxies, 2500); };

/* ---- A 服务配置（目录添加 + 已配置管理 + 测试连接） ---- */
let asTab='mailbox';
const AS_TYPE_MAP = {
  mailbox:{catalog:'mailbox_providers', settings:'mailbox_settings'},
  captcha:{catalog:'captcha_providers', settings:'captcha_settings'},
  sms:{catalog:'sms_providers', settings:'sms_settings'},
};
async function initAarSettings(){
  try{
    // 立刻渲染骨架，别让页面空着等接口
    renderAsPanel();
    ...
    window.__asOpts.sms_settings = sBox;
    // 通用策略下拉
    const sel=(el,list)=>{ if(el) el.innerHTML=(list||[]).map(v=>`<option value="${esc(v.value)}">${esc(v.label)}</option>`).join(''); };
    sel($('#ag-identity'), opts.identity_mode_options);
    sel($('#ag-oauth'), opts.oauth_provider_options);
    sel($('#ag-executor'), opts.executor_options);
    $('#ag-identity') && ($('#ag-identity').value = cfg.default_identity_provider || opts.identity_mode_options?.[0]?.value || '');
    $('#ag-oauth') && ($('#ag-oauth').value = cfg.default_oauth_provider || '');
    $('#ag-executor') && ($('#ag-executor').value = cfg.default_executor || '');
    // ChatGPT 出口配置回填
    $('#ac-cpa-url') && ($('#ac-cpa-url').value = cfg.cpa_api_url||'');
    $('#ac-cpa-key') && ($('#ac-cpa-key').value = cfg.cpa_api_key||'');
    $('#ac-any2api-url') && ($('#ac-any2api-url').value = cfg.any2api_url||'');
    $('#ac-any2api-pw') && ($('#ac-any2api-pw').value = cfg.any2api_password||'');
    renderAsPanel();
  }catch(e){ console.error('initAarSettings', e); $('#as-provider-cards').innerHTML = '<div style="grid-column:1/-1" class="dash-empty">加载失败：'+esc(e.message)+'</div>'; }
}
function renderAsPanel(){
  const isProvider = !['general','chatgpt'].includes(asTab);
  $('#as-provider-cards').style.display = isProvider ? 'grid':'none';
  $('#as-general').style.display = asTab==='general' ? 'block':'none';
  $('#as-chatgpt').style.display = asTab==='chatgpt' ? 'block':'none';
  if(!isProvider) return;
  // 数据没到就先给骨架占位，避免长时间空白
  if(!window.__asOpts){
    $('#as-provider-cards').innerHTML =
      Array.from({length:6}).map(()=>'<div class="panel aar-provider-card as-skel"><div class="skel-line w60"></div><div class="skel-line w90"></div><div class="skel-btn"></div></div>').join('');
    return;
  }
  const t = AS_TYPE_MAP[asTab];
  const catalog = window.__asOpts[t.catalog]||[];
  const configured = window.__asOpts[t.settings]||[];
  const usedKeys = new Set(configured.map(c=>c.provider_key));
  let html = '';
  // 已配置
  html += `<div style="grid-column:1/-1" class="h-label">已配置 (${configured.length})</div>`;
  html += configured.map(p=>`
    <div class="panel aar-provider-card">
      <div class="panel-head"><h3>${esc(p.display_name||p.provider_key)}</h3>${p.is_default?'<span class="badge ok">默认</span>':''}</div>
      <div class="h-label" style="margin-bottom:8px">${esc(p.description||p.catalog_label||'')}</div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <label style="display:flex;align-items:center;gap:4px;font-size:11px"><input type="checkbox" ${p.enabled?'checked':''} onchange="asToggle('${p.id}',this.checked)"> 启用</label>
        ${!p.is_default?`<button class="btn-inline" onclick="asSetDefault('${p.id}')">设为默认</button>`:''}
        <button class="btn-inline" onclick="asEdit('${p.id}')">编辑</button>
        <button class="btn-inline" style="color:#f87171" onclick="asDelete('${p.id}')">删除</button>
      </div>
    </div>`).join('') || '<div style="grid-column:1/-1" class="dash-empty">还没有配置，从下面目录里点「＋ 添加」开始</div>';
  // 可添加目录
  html += `<div style="grid-column:1/-1;margin-top:8px" class="h-label">可用服务目录 — 点「＋ 添加」配置后即可在注册时使用</div>`;
  html += catalog.filter(c=>!usedKeys.has(c.provider_key)).map(c=>`
    <div class="panel aar-provider-card">
      <div class="panel-head"><h3>${esc(c.label)}</h3></div>
      <div class="h-label" style="margin-bottom:8px">${esc(c.description||'')}</div>
      <button class="btn-run" style="padding:6px 14px;font-size:12px" onclick="asAdd('${esc(c.provider_key)}')">＋ 添加</button>
    </div>`).join('');
  $('#as-provider-cards').innerHTML = html;
}
$$('#as-tabs button').forEach(b=>b.onclick=()=>{
  asTab = b.dataset.asTab;
  $$('#as-tabs button').forEach(x=>x.classList.toggle('active', x===b));
  renderAsPanel();
});
function findCatalog(key){
  for(const t of Object.keys(AS_TYPE_MAP)){
    const hit = (window.__asOpts[AS_TYPE_MAP[t].catalog]||[]).find(c=>c.provider_key===key);
    if(hit) return {...hit, _type:t};
  }
  return null;
}
function fieldHtml(f, val){
  ...
  if(f.options) return `<label class="asset-field"><span>${esc(f.label)}</span><select data-fkey="${esc(f.key)}">${f.options.map(o=>`<option value="${esc(o.value??o)}" ${String(v)===String(o.value??o)?'selected':''}>${esc(o.label??o)}</option>`).join('')}</select></label>`;
  if(f.secret) return `<label class="asset-field"><span>${esc(f.label)}</span><input type="password" autocomplete="off" data-fkey="${esc(f.key)}" value="${esc(v)}" placeholder="${esc(f.placeholder||'')}"></label>`;
  return `<label class="asset-field"><span>${esc(f.label)}</span><input type="text" data-fkey="${esc(f.key)}" value="${esc(v)}" placeholder="${esc(f.placeholder||'')}"></label>`;
}
window.asAdd = key=>{
  const c = findCatalog(key); if(!c) return;
  $('#as-dialog-title').textContent = `添加 · ${c.label}`;
  $('#as-dialog-desc').textContent = c.description||'';
  const fields = c.fields||[];
  const authModes = c.auth_modes||[];
  $('#as-dialog-fields').innerHTML =
    (authModes.length?`<label class="asset-field"><span>认证方式</span><select id="asd-authmode">${authModes.map(m=>`<option value="${esc(m.value)}" ${(c.default_auth_mode===m.value)?'selected':''}>${esc(m.label)}</option>`).join('')}</select></label>`:'')
    + `<label class="asset-field"><span>配置名称</span><input id="asd-name" type="text" value="${esc(c.label)}"></label>`
    + fields.map(f=>fieldHtml(f,'')).join('');
  $('#as-dialog').dataset.mode='add'; $('#as-dialog').dataset.key=key; $('#as-dialog').dataset.type=c._type;
  $('#as-dialog-msg').textContent='';
  $('#as-dialog').showModal();
};
window.asEdit = id=>{
  let p=null, type=null;
  for(const t of Object.keys(AS_TYPE_MAP)){
    p = (window.__asOpts[AS_TYPE_MAP[t].settings]||[]).find(x=>String(x.id)===String(id));
    if(p){ type=t; break; }
  }
  if(!p) return;
  const c = findCatalog(p.provider_key)||{};
  $('#as-dialog-title').textContent = `编辑 · ${p.display_name||p.provider_key}`;
  $('#as-dialog-desc').textContent = c.description||'';
  const merged = {...(c.fields||[])};
  const vals = {...(p.config||{}), ...(p.auth||{})};
  $('#as-dialog-fields').innerHTML =
    ((c.auth_modes&&c.auth_modes.length)?`<label class="asset-field"><span>认证方式</span><select id="asd-authmode">${c.auth_modes.map(m=>`<option value="${esc(m.value)}" ${p.auth_mode===m.value?'selected':''}>${esc(m.label)}</option>`).join('')}</select></label>`:'')
    + `<label class="asset-field"><span>配置名称</span><input id="asd-name" type="text" value="${esc(p.display_name||'')}"></label>`
    + (c.fields||[]).map(f=>fieldHtml(f, vals[f.key])).join('');
  $('#as-dialog').dataset.mode='edit'; $('#as-dialog').dataset.id=id; $('#as-dialog').dataset.key=p.provider_key; $('#as-dialog').dataset.type=type;
  $('#as-dialog-msg').textContent='';
  $('#as-dialog').showModal();
};
function collectDialog(){
  const config={}, auth={};
  $$('#as-dialog-fields [data-fkey]').forEach(el=>{
    const cat = el.closest('.asset-field')?.querySelector('[data-cat]')?.dataset.cat;
    // 按 catalog 的 category 归类：auth 类进 auth，其余进 config
    const c = findCatalog($('#as-dialog').dataset.key);
    const meta = (c?.fields||[]).find(f=>f.key===el.dataset.fkey);
    if(meta && (meta.category==='auth')) auth[el.dataset.fkey]=el.value.trim();
    else config[el.dataset.fkey]=el.value.trim();
  });
  return {config, auth,
    name:$('#asd-name')?.value.trim()||'', authMode:$('#asd-authmode')?.value||undefined};
}
async function saveDialog(){
  const d=$('#as-dialog'); const {config,auth,name,authMode}=collectDialog();
  const body={provider_type:d.dataset.type, provider_key:d.dataset.key,
    display_name:name||d.dataset.key, enabled:true, auth_mode:authMode,
    ...(d.dataset.mode==='edit'?{id:+d.dataset.id}:{}) , config, auth};
  const r = await aarApi('/provider-settings',{method:d.dataset.mode==='edit'?'PUT':'POST',body:JSON.stringify(body)});
  d.close(); initAarSettings();
  return r;
}
$('#btn-as-save').onclick = ()=>saveDialog().catch(e=>{$('#as-dialog-msg').textContent='保存失败：'+e.message;});
$('#btn-as-test').onclick = async ()=>{
  const d=$('#as-dialog'); const {config,auth}=collectDialog();
  $('#as-dialog-msg').textContent='测试中…';
  try{
    const r = await aarApi('/provider-settings/test',{method:'POST',body:JSON.stringify({
      provider_type:d.dataset.type, provider_key:d.dataset.key, config, auth})});
    $('#as-dialog-msg').textContent = r.ok ? `✅ 连接成功${r.email?' · '+r.email:''}` : `❌ ${r.error||'测试失败'}`;
  }catch(e){ $('#as-dialog-msg').textContent='❌ '+e.message; }
};
window.asToggle = async (id,on)=>{
  for(const t of Object.keys(AS_TYPE_MAP)){
    const p=(window.__asOpts[AS_TYPE_MAP[t].settings]||[]).find(x=>String(x.id)===String(id));
    if(p){
      await aarApi('/provider-settings',{method:'PUT',body:JSON.stringify({
        id:p.id, provider_type:p.provider_type, provider_key:p.provider_key,
        display_name:p.display_name, auth_mode:p.auth_mode, enabled:on, is_default:p.is_default,
        config:p.config||{}, auth:p.auth||{}})});
      break;
    }
  }
  initAarSettings();
};
window.asSetDefault = async id=>{
  for(const t of Object.keys(AS_TYPE_MAP)){
    const p=(window.__asOpts[AS_TYPE_MAP[t].settings]||[]).find(x=>String(x.id)===String(id));
    if(p){
      await aarApi('/provider-settings',{method:'PUT',body:JSON.stringify({
        id:p.id, provider_type:p.provider_type, provider_key:p.provider_key,
        display_name:p.display_name, auth_mode:p.auth_mode, enabled:p.enabled, is_default:true,
        config:p.config||{}, auth:p.auth||{}})});
      break;
    }
  }
...
window.asDelete = async id=>{
  if(!confirm('确认删除该服务配置？')) return;
  await aarApi(`/provider-settings/${id}`, {method:'DELETE'}).catch(e=>alert('删除失败:'+e.message));
  initAarSettings();
};
$('#btn-ag-save').onclick = async ()=>{
  try{
    await aarApi('/config', {method:'PUT', body:JSON.stringify({data:{
      default_identity_provider:$('#ag-identity').value,
      default_oauth_provider:$('#ag-oauth').value,
      default_executor:$('#ag-executor').value,
    }})});
    alert('已保存');
  }catch(e){ alert('保存失败：'+e.message); }
};
$('#btn-ac-save').onclick = async ()=>{
  try{
    await aarApi('/config', {method:'PUT', body:JSON.stringify({data:{
      cpa_api_url:$('#ac-cpa-url').value.trim(),
      cpa_api_key:$('#ac-cpa-key').value.trim(),
      any2api_url:$('#ac-any2api-url').value.trim(),
      any2api_password:$('#ac-any2api-pw').value.trim(),
    }})});
    alert('已保存');
  }catch(e){ alert('保存失败：'+e.message); }
};

// 平台列表由 loadAccounts 触发加载
